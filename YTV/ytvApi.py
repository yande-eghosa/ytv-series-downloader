'''python module containing classes
and functions that does most of the webscraping and data handling'''
...
def latest():
	'''returns list of the latest episode'''
	try:
...
def compare(text,results):
	'''returns tv series matches for search entry'''
...
class BookMarks():
	'''class representing bookmarked series'''
	...
	def set_delete_set(self,dset):
		'''write to a file the bookmark thumbnail 
		to delete when next app starts'''
		if not os.path.exists(p:=book+'to_trash.txt'):
			with open(p,'w') as fd:
				fd.write(str(dset))
				return
